#!/usr/bin/env python3
"""
build_site.py — generate a self-contained, interactive single-page index.html
from the repo's markdown docs and Python lessons.

Why a generator (not a hand-written HTML): index.html embeds the real file contents
so it works offline from file:// (no fetch/CORS). Re-run this after editing any doc:

    python build_site.py

Output: index.html (single self-contained file; marked.js + highlight.js are inlined
from vendor/, so it works fully offline — just double-click it).
"""
...
SECTIONS = [
    ...
    ("walkthrough","Teacher Walkthrough",      "Guide",     "md",   "WALKTHROUGH.md",             None),
...
# Build embedded data + manifest
embeds = []
manifest = []
for sid, label, group, kind, path, lang in SECTIONS:
    raw = (ROOT / path).read_text(encoding="utf-8")
    assert "</script" not in raw.lower(), f"{path} contains a </script — would break embedding"
    embeds.append(f'<script type="text/plain" id="src-{sid}">{raw}</script>')
    manifest.append({"id": sid, "label": label, "group": group, "kind": kind,
                     "path": path, "lang": lang or "plaintext"})

EMBEDS = "\n".join(embeds)
MANIFEST = json.dumps(manifest, indent=0)
...
<style id="hljs-dark">/*HLJS_DARK*/</style>
<style id="hljs-light">/*HLJS_LIGHT*/</style>
<style>
:root{
...
header .brand{display:flex;align-items:center;gap:10px;font-weight:700;color:var(--heading);letter-spacing:.2px}
...
.doc-section{max-width:860px;margin:0 auto 56px;scroll-margin-top:78px}
.doc-section+.doc-section{border-top:1px solid var(--border);padding-top:46px}

/* markdown styling */
.doc-section h1{font-size:30px;line-height:1.2;color:var(--heading);margin:.2em 0 .5em;letter-spacing:-.01em}
...
pre{position:relative;background:var(--code-bg)!important;border:1px solid var(--border);border-radius:12px;
  padding:16px 16px;overflow:auto;margin:1.1em 0;box-shadow:var(--shadow)}
...
<header>
  ...
  <span class="meta">· Weekend Intensive</span>
...
<div class="layout">
  <nav id="side">
    <input class="search" id="search" placeholder="Filter sections…" autocomplete="off">
    <div id="navlist"></div>
  </nav>
...
const LS_PROG="agentic.progress", LS_THEME="agentic.theme";
...
function renderAll(){
  ...
  for(const it of MANIFEST){
    ...
    sec.className="doc-section"; sec.id="sec-"+it.id;
    if(it.kind==="md"){
      sec.innerHTML = window.marked ? marked.parse(raw(it.id)) : "<pre>"+esc(raw(it.id))+"</pre>";
    }else{
      sec.innerHTML =
        '<h1 class="code-title">'+it.label+'</h1>'+
        '<p class="code-path"><code>'+it.path+'</code></p>'+
        '<details class="codewrap" open><summary>source</summary>'+
        '<pre><code class="language-'+it.lang+'">'+esc(raw(it.id))+'</code></pre></details>';
    }
...
function buildNav(){
  ...
  for(const it of MANIFEST){
    if(it.group!==group){group=it.group;
      const g=document.createElement("div");g.className="navgroup";g.textContent=group;list.appendChild(g);}
    const a=document.createElement("a");
    a.className="navlink kind-"+it.kind; a.href="#sec-"+it.id; a.dataset.id=it.id;
    a.innerHTML='<input class="chk" type="checkbox"'+(p[it.id]?" checked":"")+'>'+
...
function addCopyButtons(){
  $$("pre").forEach(pre=>{
    ...
    btn.addEventListener("click",()=>{
      const code=pre.querySelector("code"); navigator.clipboard.writeText(code?code.innerText:pre.innerText);
...
/* ---- progress ring ---- */
function updateRing(){
  const p=prog(); const done=MANIFEST.filter(it=>p[it.id]).length, total=MANIFEST.length;
  const pct=Math.round(done/total*100), C=2*Math.PI*12;
  $("#ring-fg").style.strokeDashoffset = C*(1-done/total);
...
function spy(){
  ...
  const obs=new IntersectionObserver(es=>{
    es.forEach(e=>{ if(e.isIntersecting){
      ...
      const a=links.get(e.target.id.replace("sec-","")); if(a){a.classList.add("active");
...
function applyTheme(t){
  ...
  const sd=$("#hljs-dark").sheet, sl=$("#hljs-light").sheet;
  if(sd) sd.disabled = t!=="dark";
  if(sl) sl.disabled = t==="dark";
}
function theme(){
...
function chrome(){
  ...
  $("#reset-btn").addEventListener("click",()=>{
    if(confirm("Reset your progress checkmarks?")){localStorage.removeItem(LS_PROG);
      $$(".chk").forEach(c=>c.checked=false);updateRing();}
  });
}

/* ---- boot ---- */
if(window.marked) marked.setOptions({breaks:false,gfm:true});
renderAll(); buildNav(); updateRing(); spy(); search(); theme(); chrome();
</script>
</body>
</html>
"""

# Vendored libraries (inlined so index.html works fully offline from file://)
...
for blob, name in [(MARKED_JS, "marked.min.js"), (HLJS_JS, "hljs.min.js")]:
    assert "</script" not in blob.lower(), f"{name} contains </script — would break inlining"

out = (HTML
       .replace("<!--EMBEDS-->", EMBEDS)
       .replace("/*MANIFEST*/", MANIFEST)
       .replace("/*HLJS_DARK*/", HLJS_DARK)
       .replace("/*HLJS_LIGHT*/", HLJS_LIGHT)
       .replace("/*MARKED_JS*/", MARKED_JS)
       .replace("/*HLJS_JS*/", HLJS_JS))
(ROOT / "index.html").write_text(out, encoding="utf-8")
print(f"Wrote index.html ({len(out):,} bytes) from {len(SECTIONS)} sections "
      f"(libraries inlined — works offline).")
